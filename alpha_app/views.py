...
def create_order (request):
    if request.user.groups.filter(name='admin').exists():
        if request.method == "POST":
            ...
            order = Order.objects.create(article = article, customer = customer, assignee = assignee, deadline = request.POST['deadline'], estimated_budget = request.POST['budget'], status = request.POST['project-status'])
            ...
            return render(request, "project_info_page.html", {"status_choices" : Order.STATUS_CHOICES, "assignment_choices": Article.ASSIGNMENT_CHOICES, "customer_list": customer_list, "agents_list": agents_list})
    else:
